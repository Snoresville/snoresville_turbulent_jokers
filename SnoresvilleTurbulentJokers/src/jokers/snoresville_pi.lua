local pi = ""
local mult_x_base = 1
local mult_x_gain = 0.2

local function get_pi_at_index(i)
    local digit = pi:sub(i % #pi + 1, i % #pi + 1)
    return (digit == "0" and "10")
    or (digit == "1" and "Ace")
    or digit
end

local joker = {
    name = "snoresville_pi",
    slug = "snoresville_pi",
    config = {
        extra = {
            pi_index = 0
        }
    },
    spritePos = {x = 0, y = 0},
    loc_txt = {
        name = "Pi",
        text = {
            "This Joker gains {X:mult,C:white}X#1#{} Mult",
            "every time a scored card's rank",
            "matches the next rank of Pi.",
            "Next ranks are: {C:attention}#3#, #4#, #5#, #6#, #7#{}",
            "{C:inactive}(Currently {X:mult,C:white}X#2#{C:inactive} Mult)"
        }
    },
    rarity = 2,
    cost = 3,
    unlocked = true,
    discovered = true,
    blueprint_compat = false,
    eternal_compat = true,
    functions = {
        loc_def = function(self)
            return {mult_x_gain, mult_x_base + mult_x_gain * self.ability.extra.pi_index,
                get_pi_at_index(self.ability.extra.pi_index + 0),
                get_pi_at_index(self.ability.extra.pi_index + 1),
                get_pi_at_index(self.ability.extra.pi_index + 2),
                get_pi_at_index(self.ability.extra.pi_index + 3),
                get_pi_at_index(self.ability.extra.pi_index + 4)
            }
        end,
        calculate = function(self, context)
            if context.cardarea == G.play and context.individual and not context.blueprint then
                local card = context.other_card
                if not card.debuff then
                    local card_rank = (card.base.value == "10" and "0")
                    or (card.base.value == "Ace" and "1")
                    or card.base.value
                    local next_number = pi:sub(self.ability.extra.pi_index % #pi + 1, self.ability.extra.pi_index % #pi + 1)

                    if card_rank == next_number then
                        self.ability.extra.pi_index = self.ability.extra.pi_index + 1
                        return {
                            extra = {focus = self, message = localize('k_upgrade_ex'), colour = G.C.MULT},
                            colour = G.C.MULT,
                            card = self
                        }
                    end
                end
            end
            if SMODS.end_calculate_context(context) then
                if self.ability.extra.pi_index > 0 then
                    return {
                        Xmult_mod = mult_x_base + mult_x_gain * self.ability.extra.pi_index,
                        message = localize{
                            type = 'variable',
                            key = 'a_xmult',
                            vars = {
                                mult_x_base + mult_x_gain * self.ability.extra.pi_index
                            }
                        }
                    }
                end
            end
        end
    }
}

return joker